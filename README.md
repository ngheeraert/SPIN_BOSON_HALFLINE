...
A convenient dimensionless measure of coupling is $\alpha \sim Z/R_K$, where $Z$ is the waveguide impedance and $R_K = h/e^2$ is the resistance quantum. Long JJ chains can reach $Z \sim R_K$, hence $\alpha \sim 1$.
...
- mode frequencies:
  $$\omega_k = dk\,(k-\tfrac{1}{2}),\qquad k=1,\dots,N,$$
- spacing:
  $$dk = \omega_{\max}/N,$$
- coupling envelope:
...
- `nk_end_<param>_<wigxmin>_<wigxmax>.d` : a small table of $n(k)$ values after optional spatial windowing (currently outputs first few modes).
